...
const Checkout = () => {
  ...
  const getAddresses = async (token) => {
    ...
    try {
      ...
      console.log("getAddresses()", response.data);
  ...
  const validateRequest = (items, addresses) => {
    if (localStorage.getItem("balance") < getTotalCartValue(items)) {
      enqueueSnackbar(
        "You do not have enough balance in your wallet to purchase",
        { variant: "warning" }
      );
      return false;
    }
  ...
  useEffect(() => {
    const onLoadHandler = async () => {
      ...
      if (productsData && cartData) {
        ...
        console.log("checkout useEffect1", cartDetails);
        setItemsCount(getTotalItems(cartDetails));
        // console.log("checkout useEffect1 totalItems",totalItems);
      }
    };
    onLoadHandler();
  ...
  useEffect(() => {
    if (token) {
      getAddresses(token);
      console.log("checkout useEffect2", items);
      // totalItems = items?getTotalItems(items):0;
    } else {
      enqueueSnackbar("You must logged in first to access the checkout page", {
        variant: "info",
      });
      history.push("/");
    }
    ...
    <>
      ...
      <Grid container>
        ...
        <Grid item xs={12} md={3} bgcolor="#E9F5E1">
          ...
          {itemsCount > 0 ? (
            <Box bgcolor="white" borderRadius=".5rem" margin="1rem" paddinX="1rem">
              ...
              <Box display="flex" justifyContent="space-between">
                ...
                <strong> ${getTotalCartValue(items)}</strong>
              </Box>
              <Box display="flex" justifyContent="space-between">
                <Typography variant="subtitle1">ShippingCharges </Typography>
                <strong>$0</strong>
              </Box>
              <Box display="flex" justifyContent="space-between">
                <Typography variant="subtitle1">Cart Total </Typography>
                <strong> ${getTotalCartValue(items) + 0}</strong>
              </Box>
            </Box>
          ) : null}
        </Grid>
